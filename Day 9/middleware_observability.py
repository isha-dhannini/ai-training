# -*- coding: utf-8 -*-
"""middleware_observability.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1DKMJ-hqYTouuL7xJ9wuX0GizOU6M1Wnz
"""
...
# Store correlation ID per request context
...
# ---------------- OUTBOUND INJECTION ----------------
def inject_correlation_id(request: httpx.Request):
    cid = correlation_ctx.get()
    if cid:
        request.headers["X-Correlation-Id"] = cid


client = httpx.Client(event_hooks={"request": [inject_correlation_id]})
...
- Export your running `/metrics` to a free [Grafana Cloud](https://grafana.com/products/cloud/) account
  using the Prometheus remote-write endpoint
...
def safe_counter(name, desc, labels=None):
    try:
        return Counter(name, desc, labels or [])
    except ValueError:
        # already exists → reuse existing metric
        for collector in REGISTRY._collector_to_names:
            pass
        return REGISTRY._names_to_collectors.get(name)

def safe_histogram(name, desc, buckets):
    try:
...
# Payment metrics (safe init)
PAYMENT_AMOUNT = safe_histogram(
    "payment_amount",
...
ERROR_COUNT = safe_counter(
    ...
    "Number of payment processing errors",
    ["error_type"]
)
